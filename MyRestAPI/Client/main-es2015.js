...
/* harmony import */ var _syncfusion_ej2_angular_charts__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! @syncfusion/ej2-angular-charts */ "./node_modules/@syncfusion/ej2-angular-charts/@syncfusion/ej2-angular-charts.js");
...
let CrudmenuComponent = class CrudmenuComponent {
    constructor() {
        this.menuItems = [
            {
                text: "AG GRID"
...
let DataformsComponent = class DataformsComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let ProjectformComponent = class ProjectformComponent {
    ...
    addProject() {
        ...
        if (this.errors.length == 0) {
            ...
            this.ps.storeProject(obj).subscribe((data) => {
                this.projectid = "";
                this.coretech = "";
    ...
    updateProject() {
        ...
        if (this.errors.length == 0) {
            ...
            this.ps.updateProject(obj, obj.projectid).subscribe((data) => {
                alert("Data stored.!!!!");
...
let FirstchartComponent = class FirstchartComponent {
    constructor(cs) {
        this.cs = cs;
        this.chartData = [];
...
let ProductchartComponent = class ProductchartComponent {
    constructor(ss) {
        this.ss = ss;
        this.chartData = [];
        this.primaryXaxis = { valueType: 'Category' };
    ...
    ngOnInit() {
        this.ss.getProducts().subscribe((data) => this.chartData = data, () => this.chartData = []);
    }
};
...
let VisualizationComponent = class VisualizationComponent {
    constructor(ss) {
        this.ss = ss;
        this.rows = [];
    ...
    ngOnInit() {
        this.ss.getProducts().subscribe((data) => this.rows = data, () => this.rows = []);
    }
};
...
let ProductivityPipe = class ProductivityPipe {
    transform(locations, type) {
        //let result:location[]=[];
        if (type == "Low Revenue") {
...
let RatedPipe = class RatedPipe {
    transform(target, empcount) {
        if (target >= 7000)
            return target + "*";
        else
            return target;
